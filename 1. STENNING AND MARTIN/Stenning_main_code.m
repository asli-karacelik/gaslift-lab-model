...
for file_idx = 1:length(file_list)
    ...
    par = Stenning_parameter_function(S);
    ...
    for Qginj = lower_limit:increment:upper_limit % Volumetric air inflow rate (cm3/s)
        ...
        [x, fval, exitflag, output] = fsolve(@(x)Stenning_model_function(x, Qginj, par), x0, opt);
        [F, wlout] = Stenning_model_function(x, Qginj, par);
