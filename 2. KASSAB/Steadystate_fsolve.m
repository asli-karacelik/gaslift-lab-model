...
disp(S_value);
    ...
    air = data{:, 1} * (1e3/3.6e3); % Convert air flow rates
    water = data{:, 2} * (1e3/3.6e3); % Convert water flow rates
    ...
    for wginj = lower_limit:increment:upper_limit % Volumetric inflow rate (cm^3/s)
        disp(['Processing wginj: ', num2str(wginj)]); % Debugging
        
        opt = optimoptions('fsolve', 'MaxIter', 1e5, 'MaxFunEvals', 1e5);
        [x, ~, exitflag, output] = fsolve(@(x)SteadystateFcn_fsolve(x, wginj, par), x0, opt);
        
        disp(['fsolve exitflag: ', num2str(exitflag)]); % Debugging
        
        % if exitflag <= 0
        %     warning('fsolve failed for wginj = %.2f', wginj);
        %     continue;
        % end
        
        [F, wlout] = SteadystateFcn_fsolve(x, wginj, par);
        disp(['Computed wlout: ', num2str(wlout)]); % Debugging
        
        if isnan(wlout) || isinf(wlout)
            warning('Invalid wlout value for wginj = %.2f', wginj);
            continue;
        end
    ...
    wginj = lower_limit:increment:upper_limit;
    figure;
    plot(wginj, results_wlout, ':_r', 'MarkerSize', 4, 'MarkerEdge', 'b', 'LineWidth', 1);
    hold on;
    plot(air, water, '-k', 'MarkerSize', 3);
    xlabel('w_{g,inj} (cm^3/s)');
    ylabel('w_{l,out} (g/s)');
