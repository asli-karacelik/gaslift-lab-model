function [F, wlout] = SteadystateFcn_fsolve(x, Qginj, par)
%% The liquid outflow rate for the given gas inflow rate at steady-state
% Input: Gas inflow rate, Qginj (cm3/s)
% Output: Liquid outflow rate, wlout (g/s)
...
% Volumetric flow rate (cm3/s)
    ...
    Qgout = Qlout * (alphagtop) / ((1 - alphagtop)^3);
% Gas flow rate 
...
%% State equations
F(1) = wginj - wgout; % Accumulation of gas (g/s)
F(2) = wlin - wlout; % Accumulation of liquid (g/s)