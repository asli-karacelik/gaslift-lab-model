...
for file_idx = 1:length(file_list)
    ...
    par = Goharzadeh_parameter_function(S);
    
    % Read data from the current CSV file
    file_path = fullfile(folder, file_name);
    data = readtable(file_path);
    air = data{:, 1} * 1e6; % Convert air flow rates (m3/s to cm3/s)
    water = data{:, 2} * (1e6 * par.rhol); % Convert water flow rates (m3/s to g/s)
    ...
    for Qginj = lower_limit:increment:upper_limit % Volumetric air inflow rate (cm3/s)
        ...
        [x, fval, exitflag, output] = fsolve(@(x)Goharzadeh_model_function(x, Qginj, par), x0, opt);
        [F, wlout] = Goharzadeh_model_function(x, Qginj, par);
    ...
    figure;
    plot(Qginj, results_wlout, '-.r', 'LineWidth', 1);
    hold on;
    plot(air, water, '-k');
