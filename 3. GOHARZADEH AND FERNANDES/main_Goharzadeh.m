...
for file_idx = 1:length(file_list)
    ...
    par = function_parameter_Goharzadeh(S);
    ...
    for Qginj = air_min:increment:air_max % Volumetric air inflow rate (cm3/s)
        ...
        [x, fval, exitflag, output] = fsolve(@(x)function_model_Goharzadeh(x, Qginj, par), x0, opt);
        [F, wlout] = function_model_Goharzadeh(x, Qginj, par);
