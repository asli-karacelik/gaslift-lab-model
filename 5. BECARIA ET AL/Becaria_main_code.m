...
for file_idx = 1:length(file_list)
    ...
    par = Becaria_parameter_function(S);
    
    % Read data from the current CSV file
    file_path = fullfile(folder, file_name);
    data = readtable(file_path);
    air = data{:, 1} * 1e3 / 60; % Convert air flow rates (L/s to cm3/s)
    water = data{:, 2} * (1e3 / 60 * par.rhol); % Convert water flow rates (L/s to g/s)
    ...
    for Qginj = lower_limit:increment:upper_limit % Volumetric air inflow rate (cm3/s)
        ...
        [x, fval, exitflag, output] = fsolve(@(x)Becaria_model_function(x, Qginj, par), x0, opt);
        [F, wlout] = Becaria_model_function(x, Qginj, par);
    ...
    figure;
    plot(Qginj, results_wlout, '-.b', 'LineWidth', 1);
    hold on;
    plot(air, water, '-k');
